...
//
// Extrait de lib/db.php pour que la chaîne de migrations vive dans son propre
// fichier : c'est la partie qui grossit à chaque évolution du schéma, et la
// seule que l'on relit en archéologue. Chargé par lib/db.php, jamais
// directement.
//
// Pour ajouter une évolution : une entrée dans $steps (numéro suivant) + une
// fonction migration_N() ici. Chaque étape doit rester idempotente.
//
// ⚠ Piège SQLite vérifié empiriquement (voir migration_21) : pour recréer une
// table avec un schéma modifié, ne PAS faire « ALTER TABLE x RENAME TO x_old »
// puis recréer x — « PRAGMA foreign_keys = OFF » ne suffit pas à empêcher
// SQLite de réécrire les clauses REFERENCES des AUTRES tables vers x_old, qui
// devient une FK cassée une fois x_old supprimée. Créer la nouvelle table sous
// un nom temporaire (x_new), y copier les données, DROP TABLE x, puis
// ALTER TABLE x_new RENAME TO x. tests/migrations_test.php vérifie désormais
// PRAGMA foreign_key_check après toute la chaîne.
...
function migration_8(PDO $pdo): void
{
    ...
    // __DIR__ vaut lib/db/ depuis l'extraction de ce fichier : le chemin remonte
    // donc d'un cran vers lib/. Seul require relatif de toute la chaîne de
    // migrations — la suite de tests l'a signalé immédiatement après le
    // déplacement, ce qui est précisément son objet.
...
// Migration 68 : index sur login_attempts. La table n'en avait aucun — les deux
// compteurs anti-force-brute (login_failures_recent() par IP,
// login_failures_recent_email() par compte) faisaient donc un balayage complet.
// La purge opportuniste de login_record_failure() garde la table minuscule, mais
// l'index est gratuit et rend les deux lectures indépendantes de sa taille.
function migration_68(PDO $pdo): void
{
    $pdo->exec('CREATE INDEX IF NOT EXISTS idx_login_attempts_ip    ON login_attempts(ip, cree_le)');
    $pdo->exec('CREATE INDEX IF NOT EXISTS idx_login_attempts_email ON login_attempts(email, cree_le)');
}
