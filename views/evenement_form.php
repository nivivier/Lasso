...
<div class="card mt-22" id="carte-employes">
        ...
        <div class="head-actions">
            ...
            <?php if ($employesDispo): ?>
                <form method="post" action="?p=evenement_employe_lier<?= $depuisQs ?>" class="linked-add"
                        data-ajout="#employes-lies" data-ajout-vide="#employes-lies-vide" data-ajout-message="Employé ajouté.">
                    <input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>">
                    <input type="hidden" name="id" value="<?= (int) $id ?>">
    ...
    <?php if ($prodExterne): ?>
        ...
        <table class="list evenement-employes">
            ...
            <tbody id="employes-lies">
            <?php if (!$employesLies): ?>
                <tr id="employes-lies-vide"><td colspan="2" class="muted small">Aucun employé lié.</td></tr>
            <?php endif; ?>
            <?php foreach ($employesLies as $emp): ?>
                <?php require __DIR__ . '/_evenement_employe_ligne_externe.php'; ?>
            <?php endforeach; ?>
            </tbody>
        </table>
        </div>
    <?php else: ?>
        ...
        <table class="list evenement-employes">
            ...
            <tbody id="employes-lies">
            <?php if (!$employesLies): ?>
                <tr id="employes-lies-vide"><td colspan="<?= $colspanMsg + 2 ?>" class="muted small">Aucun employé lié.</td></tr>
            <?php endif; ?>
            <?php foreach ($employesLies as $emp): ?>
                <?php require __DIR__ . '/_evenement_employe_ligne.php'; ?>
            <?php endforeach; ?>
            </tbody>
        </table>
        </div>
...
<div class="card">
    ...
    <?php // Le tableau est rendu même vide, et c'est une LIGNE qui porte le
          // « aucune facture » : une facture liée en arrière-plan s'insère alors
          // dans un <tbody> qui existe déjà (docs/UI.md § 5). ?>
    <table class="list">
        <thead><tr><th>Numéro</th><th>Structure</th><th class="num">Montant</th><th>Statut</th><th></th></tr></thead>
        <tbody id="factures-liees">
        <?php if (!$factures): ?>
            <tr id="factures-liees-vide"><td colspan="5" class="muted small">Aucune facture liée à cet événement.</td></tr>
        <?php endif; ?>
        <?php foreach ($factures as $fa): ?>
            <?php require __DIR__ . '/_evenement_facture_ligne.php'; ?>
        <?php endforeach; ?>
        </tbody>
    </table>
...
(function () {
    ...
    //
    // Écouteurs DÉLÉGUÉS : une ligne ajoutée en arrière-plan (data-ajout) doit
    // calculer son total comme les autres, sans qu'on rebranche quoi que ce soit.
    const epfNum = v => parseFloat((v || '').toString().replace(',', '.')) || 0;
    const epfTotal = tr => {
        ...
        const montant = (opt ? epfNum(opt.dataset.h) : 0) * epfNum(qte.value)
            * (choix.value === 'autre' ? epfNum(manuel.value) : epfNum(choix.value));
        total.textContent = montant > 0 ? (Math.round(montant * 100) / 100).toFixed(2) + ' CHF' : '—';
    };
    ['change', 'input'].forEach(evt => document.addEventListener(evt, ev => {
        const tr = ev.target.closest('.evenement-employes tbody tr');
        if (tr && ev.target.closest('.l-unite, .l-qte, .l-taux-choix, .l-taux-manuel')) epfTotal(tr);
    }));
    document.querySelectorAll('.evenement-employes tbody tr').forEach(epfTotal);
    // Une ligne arrivée après coup fait son premier calcul en entrant.
    document.addEventListener('lasso:ligne-ajoutee', ev => {
        if (ev.detail?.matches?.('.evenement-employes tbody tr')) epfTotal(ev.detail);
    });
