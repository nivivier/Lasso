...
<div class="card form" id="import-unifie" data-type-actif="<?= e((string) $typeActif) ?>">
    ...
    <form method="post" action="?p=import_fiches" enctype="multipart/form-data" id="import-form">
        ...
        <p class="muted small import-aide" data-for="evenements" hidden>Agenda de tournée en <strong>CSV</strong> (colonnes : <code>date, ville, region, pays, lieu, details, type, statut, lien, lien_texte</code> — ordre libre, seules <code>date</code> et <code>ville</code> obligatoires, date JJ/MM/AAAA). Un événement à la même date/ville/salle est <strong>ignoré</strong>. Les événements importés sont créés <strong>non répertoriés</strong>.
