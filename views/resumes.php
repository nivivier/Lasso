...
$dash_svg = function (array $series): string {
    ...
    foreach ($annees as $i => $a) {
        ...
        $o .= '<text x="' . $x . '" y="' . ($H - $mb + 16) . '" text-anchor="middle"'
            . ' font-size="11" fill="#6b7280" font-family="inherit">' . (int) $a . '</text>';
        // Tick vertical
    ...
    $series_def = [
        ['key' => 'patrimoine', 'color' => '#6d4ade', 'dash' => '',    'width' => '2'],
        ['key' => 'produits',   'color' => '#0c9486', 'dash' => '',    'width' => '2'],
        ['key' => 'charges',    'color' => '#e0473c', 'dash' => '',    'width' => '2'],
        ['key' => 'resultat',   'color' => '#292150', 'dash' => '6,3', 'width' => '2'],
    ];
    if ($n > 1) {
        foreach ($series_def as $s) {
            $dash = $s['dash'] !== '' ? ' stroke-dasharray="' . $s['dash'] . '"' : '';
            $o   .= '<polyline points="' . $pts($s['key']) . '" fill="none"'
    ...
    foreach ($items as $idx => [$label, $col, $dash]) {
        ...
        $o .= '<text x="' . $x . '" y="' . ($ly + 4) . '" font-size="10.5" fill="#6b7280"'
...
<?php if (module_actif('compta') && count($comptaSeries) >= 1): ?>
<div class="card dash-chart-card">
    <div class="section-head"><h2 class="mt-0">Évolution financière</h2>
        <a href="?p=compta_bilan" class="btn ghost btn-sm ml-auto">Comptes annuels</a>
    </div>
    <?= $dash_svg($comptaSeries) ?>
</div>
<?php endif; ?>

<?php $dashModuleActif = module_actif('salaires') || module_actif('facturation'); ?>
<?php if (!$dashModuleActif): ?>
    <p class="muted">Aucun module actif n'alimente le tableau de bord pour l'instant. Active
    Fiches de salaire ou Facturation dans <a href="?p=parametres_modules">Paramètres → Modules</a>.</p>
<?php else: ?>
<div class="dash-cols">
